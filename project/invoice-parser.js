...
async function pdfText(dataUrl) {
  ...
  for (let p = 1; p <= doc.numPages; p++) {
    ...
    // Rebuild visual lines by rounding the y coordinate of each text run.
    const rows = new Map();
...
export async function readInvoiceText(dataUrl, mimeType, onProgress) {
...
export function findInvoiceDate(text) {
  ...
  // 05/17/2024 or 17/05/2024 — prefer month-first, fall back to day-first
  ...
  // Prefer the earliest-appearing plausible date, but never a future one.
...
function cleanPart(line, moneyTokens) {
  ...
  s = s.replace(/^\s*\d+(?:\.\d+)?\s+/, ' ');       // leading line number / qty
...
function findQty(line) {
  ...
  if (m) return m[1];
  m = line.match(/^\s*(?:\d{1,3}\s+)?(\d{1,4}(?:\.\d{1,2})?)\s+[A-Za-z]/); // qty in a leading column
...
export function parseInvoiceLines(text) {
  ...
  rawLines.forEach((line) => {
    ...
    if (letters < 3) return;                          // numeric-only row
    ...
    if (nums.length === 1) {
      ...
      // Last money token on a line item is the extended amount.
      total = nums[nums.length - 1];
      unit = nums[nums.length - 2];
      // Sanity check: if qty × unit doesn't come near the total, trust qty × unit
      // only when the total looks like a running balance instead.
